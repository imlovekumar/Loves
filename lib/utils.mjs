...
async function verify_booking_params(params) {
    ...
    for (let passenger of params.passengers) {
        ...
        } else if (typeof passenger.food !== 'string' || !["V", "N", "D", "E", ""].includes(passenger.food)) {
    ...
    else if (["GN"].includes(params.quota) && ["Y"].includes(params.firstday)){
        params.ticket_time = 120;
    }    
    else{
...
async function initialize_booking_variables(params){
    ...
    params.mancap = params.params.ManualCaptcha;
    params.premcap = params.params.PremiumCaptcha;
