...
async function maintainance()
{
    ...
    targetTime.setUTCHours(18, 10, 0, 0);
    let stoptime = new Date(currentTime);
    stoptime.setUTCHours(18, 50, 0, 0);
    if (currentTime > targetTime && currentTime < stoptime)
            {
                const balance_time = stoptime - currentTime;
                for (let i=balance_time* 0.001 ; i>=1;i=i-1)
                {
                    logUpdate(chalk.whiteBright(`\n Load IRCTC @ (00:20 AM)\n Maintainance Time 23:40 Hrs To 00:20 Hrs\n`,`Wait For`,chalk.red(`${i}`),`seconds to proceed...`));
                    await sleep(1000);
                };
...
async function sleep_for_availability_check(ticket_time){
    ...
    else if (ticket_time === 120){
        const date_time_string = await get_current_time_api();
        const currentTime = new Date(date_time_string);
        let targetTime = new Date(currentTime);
        targetTime.setUTCHours(2, 30, 2, 0);
        if (currentTime < targetTime) {
            const balance_time = targetTime - currentTime;
            for (let i=balance_time* 0.001 ; i>=1;i=i-1)
                { 
...
export {sleep,maintainance, psg_input_wait, sleep_for_login, sleep_for_availability_check, start_before_3_minutes,get_current_time_api};
