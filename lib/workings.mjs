...
import {sleep,get_current_time_api,psg_input_wait, sleep_for_login,maintainance, sleep_for_availability_check} from "./time_work.mjs";
...
class IRCTC{
            ...
            if (this.act["error"] === "unauthorized" && this.act["error_description"] === "Invalid Captcha....") 
                {
                    ...
                    writeFileSync(path, this.captchaQuestion, 'base64');
                    console.clear();                
                    await this.clicking_sign_button();  
                } 
    ...
    async get_trains()
    {
        ...
        console.log(' ',this.train_number,trainname,'\n ',this.from_stn,'-',
    ...
    async get_class_availability(){
        ...
        {
            const wltype = data.avlDayList[0].availablityType;
            if (wltype === '3' || wltype === '0' || wltype === '2')
                {
                    if(wltype === '2')
                    {
                        logUpdate('Availability :',chalk.yellow(data.avlDayList[0].availablityStatus));
                    }
                    else
                    {
                        logUpdate('Availability :',chalk.red(data.avlDayList[0].availablityStatus));
                    }
                    console.log('Tickets are unavailable, Exiting Booking Process');
                    await sleep(2000);
                    process.exit();
                }
                else
                {
                    //logUpdate(' Availability :',chalk.green(data.avlDayList[0].availablityStatus),'...');
    ...
    async confirm_booking(){
        ...
        while (this.booking_captcha_response !== "SUCCESS")
        {
            await this.answer_captcha();
            headers['spa-csrf-token'] = this.csrf_token;
            logUpdate(' Verifying Final Captcha...')
    ...
    async select_paytm_upi_gateway(){
        ...
        logUpdate(chalk.redBright(" Selecting Paytm UPI Gateway..."));
        const response = await this.axios_instance.post(
        ...
        if (!response.data.hasOwnProperty("paramList")){
            ...
            logUpdate(" Recieved Paytm UPI Gateway Data...");
    ...
    async get_gateway_redirect_params(){
        ...
        logUpdate(" Redirecting To Paytm UPI Gateway...");
        const response = await this.axios_instance.post(
    ...
    async going_to_paytm(){
        ...
        logUpdate(" Reached Paytm UPI Gateway...");
    ...
    async send_upi_collect_request(){
        ...
        logUpdate(chalk.green(" Sending UPI Collect Request ..."));
        const response = await this.axios_instance.post(
            action_url,
            data_string,
            {
    ...
    async get_upi_transaction_status(){
        ...
        const url = new URL(action_url);        headers["Host"] = url.origin.split('//')[1];
        headers["Content-Length"] = data_string.length.toString();
        while (true){
    ...
    async last_transaction_depth(){
        ...
        this.csrf_token = response.headers["csrf-token"];
        return response.data.bookingResponseList[0].qrCodeText;
    }
    async last_transaction(params){
        ...
        this.mancap = params.ManualCaptcha;
        this.params = {};
